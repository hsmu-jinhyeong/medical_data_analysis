...
7가지 모델을 동일한 데이터셋(학습/테스트 8:2, random_state=42)으로 학습하여 성능을 비교하였다.  
로지스틱 회귀·KNN은 StandardScaler 정규화 적용, 나머지 모델은 원본 피처 사용.

| 모델 | 정확도 | 정밀도 | 재현율 | F1 | AUC-ROC | 학습시간 |
|---|---|---|---|---|---|---|
| 로지스틱 회귀 | 0.7393 | 0.7568 | 0.6970 | 0.7257 | 0.8129 | 0.07s |
| Decision Tree | 0.7336 | 0.7608 | 0.6731 | 0.7143 | 0.8003 | 0.30s |
| 랜덤 포레스트 | 0.7428 | 0.7524 | 0.7156 | 0.7335 | 0.8105 | 9.26s |
| **XGBoost** | **0.7477** | **0.7566** | **0.7226** | **0.7392** | **0.8182** | **1.03s** |
| 🏆 **LightGBM** | **0.7487** | **0.7560** | **0.7265** | **0.7410** | **0.8199** | **0.93s** |
| KNN | 0.7074 | 0.7091 | 0.6930 | 0.7009 | 0.7574 | 0.01s |
| Naive Bayes | 0.7224 | 0.7557 | 0.6486 | 0.6981 | 0.7899 | 0.02s |

**분석 결과 요약**

- **🏆 최고 성능: LightGBM** — AUC-ROC 0.8199, F1 0.7410으로 전체 1위
- **XGBoost** — AUC-ROC 0.8182로 LightGBM과 근소한 차이(0.0017)로 2위. 학습 시간도 유사(0.93s vs 1.03s)
- **랜덤 포레스트** — 성능은 양호하나 학습 시간이 9.26s로 가장 오래 걸림
- **로지스틱 회귀** — 단순 선형 모델임에도 AUC-ROC 0.8129로 준수한 성능. 빠른 학습 속도(0.07s)가 장점
- **Decision Tree** — 단일 트리의 한계로 전체 중 하위권이나 AUC 0.8003으로 기준선(0.5)은 크게 상회
- **KNN** — AUC-ROC 0.7574로 전체 최하위. 대용량 데이터에서 거리 계산 기반 모델의 한계
- **Naive Bayes** — 변수 간 독립성 가정으로 인해 상관된 임상 수치들이 많은 이 데이터셋에서 성능 제한

**최종 모델 선택: XGBoost 유지**

LightGBM이 근소하게 높은 성능을 보이나, XGBoost와의 차이가 미미(AUC 0.0017, F1 0.0018)하고  
피처 중요도 해석과 코드 구조의 일관성을 고려해 최종 모델은 XGBoost로 유지한다.
